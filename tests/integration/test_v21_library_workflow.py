...
def test_library_report_shows_a_style_specific_3d_model(client, django_user_model):
    ...
    assert "3D 摆放效果" in page
    assert "report-model-shelf" in page
...
def test_report_uses_the_active_theme_surface_with_a_lightweight_data_grid():
    ...
    assert ".report-model-shelf { display: grid;" in css
    assert ".generation-settings-layout { align-items: start; }" in css
    assert 'button.closest(".report-model-shelf")' in Path("static/js/creation.js").read_text()
