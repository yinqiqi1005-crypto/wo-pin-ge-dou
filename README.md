...
**当前状态：作品集 Demo 已完成并可本地运行。** 离线演示不需要付费模型、真人调研、实体拼豆或公网部署。一次性演示准备、面试流程和可表达边界见 [作品集 Demo 交付说明](docs/portfolio-demo.md)。真人评审与生产发布工具继续保留为未来商业验证，不阻止作品集展示。

## 已实现能力

- JPG/PNG 上传、质量与主体分析、规则降级和可配置真实多模态适配器；
- 十种 29×29 拼板组合与 14×14 图标模式、脸部细节／整体构图模式、12/24/36 色，透明格、色板映射和孤立格清理；
...
- 四套可切换主题、个人资料和可编辑分类；前台简中、英文、日文、韩文关键功能文案，后台保持中文；
- 面向求职演示的 `POST /api/v1/generation-tasks/{task_id}/confirm/`，具备认证、归属、额度、幂等和高频限制；
...
作品集模式使用独立完成标准。准备演示数据后运行：

```bash
uv run python manage.py check_portfolio_demo
```

命令通过即表示本地求职演示所需页面、账号、配置、素材和确定性图纸核心均已就绪。需要商业上线时再使用后文更严格的 `check_release_quality`，两者不是同一个门槛。
...
如需把已存在的本地演示账户设为管理员和 Pro 会员（不会读取或修改密码），运行：

```bash
uv run python manage.py grant_demo_admin --username abo
```
...
这一节不影响作品集 Demo 完成或本地展示。未来需要主张真实用户效果、实体可制作性或公网生产就绪时，再完成以下商业验证。
